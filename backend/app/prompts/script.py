...
def build_prompt(brief: dict[str, Any], idea: dict[str, Any], req: dict[str, Any]) -> str:
    ...
    return (
        ...
        "presenter would speak. LINE LENGTH (critical for lip-sync): keep EACH scene's "
        "'voiceover' to ONE short, punchy line of about 8-15 words that a person can say "
        "naturally in ~6-8 seconds — short lines lip-sync cleanly, long lines get rushed and "
        "break the sync. Start with a strong hook in the first scene. Each "
